...
import { AvatarDropdown, Button, Divider, Dropdown, DropdownItem, Search } from '~/components'
...
import { FaCartShoppingIcon, FaUserIcon, IoMenuIcon } from '~/utils/icons'
...
function Header() {
  ...
  useEffect(() => {
    window.addEventListener('scroll', function () {
      mainHeaderRef.current?.classList?.toggle(styles.active, window.scrollY > 71)
    })
  }, [])
  ...
  return (
    <>
      <header>
        <div className="bg-primary-400 text-white relative z-50" ref={mainHeaderRef}>
          <div
            className="container py-2 md:py-3 lg:py-4 flex items-center
                ...
                <Dropdown
                  label={
                    <Link to={routesConfig.cart} className="py-2 flex">
                      ...
                      <span
                        className="hidden md:flex relative -top-[10px] -left-[10px]
                      ...
                      <div className="flex items-center justify-between p-3">
                        <div className="text-black/60 text-[12px]">
                          {cart.products?.length > 5
                            ? 'There are 5 more products added'
                            : `There are ${cart.products?.length} products added`}
                        </div>
                ...
                >
                  {cart.products.slice(0, 5).map(({ product, variantId }, index) => {
                    ...
                    return (
                      ...
                      >
                        <img src={variant.images[0]} className="w-[50px] h-[50px] object-cover" />
                        <div className="flex-1 flex flex-col md:flex-row gap-[6px] md:gap-0">
                          <div className="flex-1 flex flex-col gap-[6px]">
                            ...
                            <p className="text-gray-500 text-[12px]">Variant: {variant.name}</p>
                          </div>
                          <div>{formatCash(product.price)}</div>
                        </div>
                      </DropdownItem>
                    )
                  })}
                ...
                {user._id ? (
                  <div className="hidden md:block">
                    <AvatarDropdown
                      ...
                      hover
                    >
                      <DropdownItem link={routesConfig.profile}>My Account</DropdownItem>
                      <DropdownItem link={routesConfig.orders}>My Orders</DropdownItem>
                      <Divider />
                      <DropdownItem onClick={handleSignOut}>Sign Out</DropdownItem>
                    </AvatarDropdown>
                  </div>
                ) : (
                  <Link to={routesConfig.signIn} className="flex items-center gap-2">
                    <Button
                      icon={
                      ...
                      rounded
                    >
                      <span className="text-[12px] md:text-[14px]">Sign In</span>
                    </Button>
                  </Link>
                )}
                <LanguageSwitcher />
